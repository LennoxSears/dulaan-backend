...
// Test LLM-only endpoint (bypasses Speech API for testing)
exports.testLLMOnly = onRequest(
    {
        region: "europe-west1",
        invoker: "public",
        cors: {
    ...
    async (req, res) => {
        try {
            ...
            const { msgHis, currentPwm, testTranscript } = req.body;
            const transcript = testTranscript || "Turn it up";

            // Process with LLM for motor control
            ...
            const prompt = `You are controlling a motor device with PWM values (0-255). Current PWM: ${currentPwm || 100}
...
Be conversational and helpful. If the user asks to increase/decrease, adjust relative to current value.`;
            ...
            try {
                ...
                return res.status(500).json({
                    ...
                    newPwmValue: currentPwm || 100,
                    msgHis: msgHis || []
                });
            }

            const newPwmValue = Math.max(0, Math.min(255, parseInt(llmResponse.pwm) || currentPwm || 100));
            ...
            res.json({
                ...
                previousPwm: currentPwm || 100,
                msgHis: trimmedMsgHis,
                testMode: true
            });

        } catch (error) {
            logger.error('Test LLM error:', error);
            res.status(500).json({
                success: false,
